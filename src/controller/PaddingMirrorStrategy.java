...
public class PaddingMirrorStrategy extends PaddingStrategy {
	...
	public Pixel pixelAt(ImageX image, int x, int y) {
		String debug = "ImgX: " + image.getImageWidth() + "x" + image.getImageHeight() + "(" + x + ", " + y + ")";
		
		x = Math.abs(x);
		y = Math.abs(y);
		
		if(x >= image.getImageWidth())
			x = image.getImageWidth() - 2 - (x - image.getImageWidth());
		
		if(y >= image.getImageHeight())
			y = image.getImageHeight() - 2 - (y - image.getImageHeight());
	...
	public PixelDouble pixelAt(ImageDouble image, int x, int y) {
		String debug = "ImgDbl: " + image.getImageWidth() + "x" + image.getImageHeight() + "(" + x + ", " + y + ")";
